...
const commonItems: GachaItem[] = [
  ...
  },
  {
    ...
    rarity: 'Phổ thông',
    type: 'armor',
    desc: '+10% linh thạch từ nhiệm vụ',
  },
  {
    name: 'Linh Văn Thạch',
    icon: '🪨',
    rarity: 'Phổ thông',
    type: 'material',
    desc: 'Nguyên liệu nâng cấp',
  },
  {
    name: 'Ngân Linh Thạch',
    icon: '🪩',
    rarity: 'Phổ thông',
    type: 'material',
    desc: 'Nguyên liệu nâng cấp',
...
const weaponPool: GachaItem[] = [
  ...commonItems,
  {
    ...
    rarity: 'Hiếm',
    type: 'weapon',
    desc: 'Công kích linh lực +800',
  },
  {
    name: 'Ngân Nguyệt Kiếm',
    icon: '🌙',
    rarity: 'Hiếm',
    type: 'weapon',
    desc: 'Nguyệt hoa kiếm, giảm 20% cooldown',
  },
  {
    ...
    rarity: 'Sử thi',
    type: 'weapon',
    desc: 'Hỏa thuộc tính, thiêu đốt kẻ thù',
  },
  {
    name: 'Thiên Lôi Cung',
    icon: '⚡',
    rarity: 'Sử thi',
    type: 'weapon',
    desc: 'Lôi thuộc tính cực mạnh',
  },
  {
    name: 'Vạn Cổ Thần Kiếm',
    icon: '✨',
    rarity: 'Thần thoại',
    type: 'weapon',
    desc: 'Kiếm vô song thiên hạ, power ×3',
  },
  {
    name: 'Hồng Mông Trượng',
    icon: '🔱',
    rarity: 'Thần thoại',
    type: 'weapon',
    desc: 'Vũ khí khai thiên tịch địa',
  },
];
...
const pillPool: GachaItem[] = [
  ...
  { name: 'Huyết Long Đan', icon: '🔴', rarity: 'Hiếm', type: 'pill', desc: 'HP tối đa +5000' },
  {
    ...
    rarity: 'Sử thi',
    type: 'pill',
    desc: '+100% linh lực, đột phá Hóa Thần',
  },
  {
    ...
    rarity: 'Sử thi',
    type: 'pill',
    desc: 'Đột phá 9 lần cảnh giới liên tiếp',
  },
  {
    name: 'Thiên Địa Linh Đan',
    icon: '🌟',
    rarity: 'Thần thoại',
    type: 'pill',
    desc: 'Trực tiếp lên cảnh giới Luyện Hư',
  },
  {
    name: 'Hỗn Độn Nguyên Đan',
    icon: '🫧',
    rarity: 'Thần thoại',
    type: 'pill',
    desc: 'Đôi lộc linh thạch trong 30 ngày',
  },
];
...
const rarityConfig: Record<
  Rarity,
  { weight: number; color: string; glow: string; badge: string; star: number }
> = {
  'Phổ thông': {
    weight: 65,
...
function roll(pool: GachaItem[], count: number): GachaItem[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const [r, cfg] of Object.entries(rarityConfig) as [
      Rarity,
      (typeof rarityConfig)[Rarity],
    ][]) {
...
export default function VongQuayPage() {
  ...
  const doPull = useCallback(
    async (count: 1 | 10) => {
      ...
      // Add sparkles effect
      const newSparkles = Array.from({ length: count * 3 }, (_, i) => ({
        id: Date.now() + i,
        x: Math.random() * 100,
        y: Math.random() * 100,
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-3 sm:px-6 lg:px-8">
        ...
        {/* ── Banner selector ── */}
        <div className="mb-5 grid grid-cols-1 gap-2 sm:mb-7 sm:grid-cols-3 sm:gap-3">
          {banners.map((b) => (
            <button
              key={b.id}
              onClick={() => {
          ...
          <div className="relative px-8 py-10 text-center">
            ...
            <div className="relative my-8 flex justify-center">
              {sparkles.map((sparkle) => (
                <div
                  key={sparkle.id}
                  className="absolute h-2 w-2 animate-ping rounded-full bg-amber-300"
                  style={{
                    top: `${sparkle.y}%`,
                    left: `${sparkle.x}%`,
              ...
              <div
                className={`relative flex h-40 w-40 items-center justify-center rounded-full transition-all duration-700 ${pulling ? 'scale-110 animate-spin' : 'hover:scale-105'}`}
                style={{
                  background: pulling
                    ? 'radial-gradient(circle, rgba(255,200,100,0.4) 0%, rgba(180,100,255,0.6) 40%, rgba(60,20,120,0.8) 70%, rgba(10,5,30,0.95) 100%)'
                    : 'radial-gradient(circle, rgba(180,100,255,0.3) 0%, rgba(60,20,120,0.6) 60%, rgba(10,5,30,0.9) 100%)',
                  boxShadow: pulling
                    ? '0 0 80px rgba(255,200,100,0.6), 0 0 160px rgba(180,100,255,0.3), 0 0 240px rgba(160,80,255,0.15)'
                    : '0 0 60px rgba(160,80,255,0.4), 0 0 120px rgba(160,80,255,0.15)',
                  border: '2px solid rgba(255,255,255,0.1)',
                }}
              >
                <div className="absolute inset-0 animate-pulse rounded-full border-2 border-purple-400/20" />
                <div
                  className="absolute inset-0 animate-spin rounded-full bg-linear-to-r from-transparent via-white/5 to-transparent"
                  style={{ animationDuration: '3s' }}
                />
                <div className="relative text-6xl transition-transform duration-300">
                  {pulling ? (
                    <div className="animate-bounce">✨</div>
                  ) : (
                    <div className="transition-transform hover:scale-110">{activeBanner.icon}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center gap-4">
              ...
              >
                ...
                <Gift className="relative h-4 w-4 text-amber-300" />
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-5 gap-3 p-5 sm:grid-cols-5 md:grid-cols-10">
              {results.map((item, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`group relative flex flex-col items-center gap-1.5 rounded-2xl border p-3 text-center transition-all ${item.rarity === 'Thần thoại'
                        ? 'scale-105 border-amber-500/60 bg-amber-900/20 shadow-xl shadow-amber-900/40'
                        : item.rarity === 'Sử thi'
                  ...
                  >
                    ...
                    <div
                      className={`flex h-10 w-10 items-center justify-center rounded-xl text-2xl shadow-md ${cfg.glow}`}
                      style={{ background: 'rgba(255,255,255,0.05)' }}
                    >
                      {item.icon}
                    </div>
                    <p className={`text-[10px] leading-tight font-bold ${cfg.color}`}>
                      {item.name}
                    </p>
            ...
            <div className="flex flex-wrap gap-2 p-4">
              {history.slice(0, 20).map(({ item }, i) => {
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${item.rarity === 'Thần thoại'
